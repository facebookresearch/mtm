...
class SequenceDataset:
    ...
    def trajectory_statistics(self) -> Dict[str, DataStatistics]:
        ...
        if self.use_avg:
            ...
            save_path = Path(
                f"/checkpoint/philippwu/d4rl_statistics_{str(self.dataset.env)}_avg.pkl"
            )
        else:
            if self.discount == 1.0:
                save_path = Path(
                    f"/checkpoint/philippwu/d4rl_statistics_{str(self.dataset.env)}_d=1.0.pkl"
                )
            elif self.discount == 0.99:
                save_path = Path(
                    f"/checkpoint/philippwu/d4rl_statistics_{str(self.dataset.env)}.pkl"
                )
            else:
