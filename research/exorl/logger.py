...
from research.jax_models.utils import WandBLogger, WandBLoggerConfig
...
class Logger(object):
    def __init__(self, log_dir, use_tb, config, name=None, offline=False):
        ...
        self._sw = WandBLogger(
            WandBLoggerConfig(
                experiment_id=name, project="rep_exorl_experiments_test_sa"
            ),
            config,
            enable=use_tb,
        )
    ...
    def _log(self, key, value):
        if key.startswith("train") or key.startswith("eval"):
            ...
            print(f"{key}: {value} - not logged")

    def log_metrics(self, metrics, step, ty):
        if len(metrics) == 0:
            return

        self._sw.log({f"{ty}/{key}": value for key, value in metrics.items()}, step)

        for key, value in metrics.items():
            self._log(f"{ty}/{key}", value)

    def log_wandb(self, name, image, step):
        self._sw.log({name: image}, step)
